...
from maxwell.server.reporter import Reporter
...
class App(FastAPI):
    ...
    def register(self):
        logger.info("App register: report inited22222")
        if self.__reporter is None:
            logger.info("App register: report inited")
            self.__reporter = Reporter(self, None)
        ...
        async def websocket_endpoint(websocket: WebSocket):
            ...
            try:
                while True:
                    ...
                    asyncio.ensure_future(self.__handle_request(websocket, data))
            except WebSocketDisconnect:
                logger.warning("Connection was closed.")

    async def __handle_request(self, websocket, data):
        try:
            ...
            logger.warning("Failed to handle request")
